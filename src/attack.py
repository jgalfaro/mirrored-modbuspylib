...
class MITM:
    def __init__(self,targetIP, victimIP,gatewayIP):
        ...
        # Get the bridge's barrier infos
...
"""
def MBfuzzing(ip, test, quantity=1, unitId=255):
    ...
    for i in range (0, quantity):
        ...
        pkt = None
        pktS = None
        ADUGeneric = ModbusADU_Request(transId=getTransId(), unitId=unitId)
...
#        print "Test " + str(test)
        ...
        # Fuzz write_single_coils_request        
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId) / fuzz(ModbusPDU05_Write_Single_Coil_Request(funcCode=5))
        elif test == 3:
            # Test ADU transId always at "0"
            pkt = fuzz(ModbusADU_Request(transId=0, protoId=0, unitId=unitId)) / PDUGeneric
        elif test == 4:
            # Test ADU proto Id must be at "0"
            pkt = fuzz(ModbusADU_Request(transId=0, unitId=unitId)) / PDUGeneric
        elif test == 5:
            ...
            # Test PDU FC
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId) / ModbusPDU00_Generic_Request(funcCode=RandNum(0, 255))
        elif test == 7:
            # ADU Corrects Multiple dans une requete
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\x00\x06' + unitidStr + PDUGenericStr + '\x00\x01\x00\x00\x00\x06' + unitidStr + PDUGenericStr)
        elif test == 8:
            # ADU with incorrect len at 0
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\x00\x00' + unitidStr + PDUGenericStr)

        elif test == 9:
            # ADU with incorrect len at 1
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\x00\x01' + unitidStr + PDUGenericStr)

        elif test == 10:
            # ADU with incorrect len (> real size)
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\x00\x08' + unitidStr + PDUGenericStr)

        elif test == 11:
            # ADU with incorrect len (1 < xxx < real size)
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\x00\x05' + unitidStr + PDUGenericStr)

        elif test == 12:
            # ADU with incorrect len too big
            pktS = array.array('B')
            pktS.fromstring('\x00\x01\x00\x00\xFF\xFF' + unitidStr + PDUGenericStr)
            
        elif test == 13:
            # No PDU
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId) 
        elif test == 14:
            # Writing more than possible (N > max address)
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId)/ModbusPDU0F_Write_Multiple_Coils_Request(startingAddr=0xFFFF, quantityOutput=6)            
        
        elif test == 15:
            # Fuzz on outputValue (only 0x0000 and 0xFF00 expected)
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId)/fuzz(ModbusPDU05_Write_Single_Coil_Request(funcCode=5, outputAddr=0))
        elif test == 16:
            # Quantity at 0
            pkt = ModbusADU_Request(transId=getTransId(), unitId=unitId)/ModbusPDU01_Read_Coils_Request(startAddr=0, quantity=0)
        elif test == 17:
            # Incoherent Quantity 
            pktS = array.array('B')
...
def fuzzAll(ip, quantity=50, unitId=255):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q", "--quantity", help="Quantity", dest='quantity', type=int, default=1)
    parser.add_argument("-u", "--unitid", help="Modbus Unit Id", dest='unitid', type=int, default=255)
    ...
    if args.mode == "scanNetwork":
        ...
        MBfuzzing(args.target, args.test, args.quantity, args.unitid)
    elif args.mode == "fuzzAll":
        fuzzAll(args.target, args.quantity, args.unitid)
